...
if __name__ == '__main__':
    ...
    filter_bank = generate_filter_bank_using_skimage(6, sigmas=(1, 3), freqs=(0.05, 0.25))
    # visualize_filter_bank(filter_bank, 6, 4)

    # pattern_features = pattern_features_of_image_from_filter_bank(raw_img, filter_bank)
    pattern_features = pattern_features_of_image_using_skimage(raw_img, N=6, sigmas=(1, 3), freqs=(0.05, 0.25),
                                                               mode='reflect')

    fvs_img = feature_vector_set_of_image(raw_img.shape, pattern_features, len(filter_bank), window_size=7)
    # visualize_feature_vector_set_of_image(fvs_img, n_clusters=4)

    fv_user = feature_vector_user(raw_img, scribbled_img, fvs_img)
